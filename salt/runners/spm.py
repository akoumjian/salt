...
def fetch_pkg(pkg_name, url):
    ...
    if pkg_ext == 'git':
        try:
            ...
            ## it just prints "git" to console otherwise
            _salt_print('Cloning {0} from {1}'.format(pkg_name, url))
...
def _salt_print(msg=''):
    salt.output.display_output(msg, 'raw', __opts__)


def remove(pkg_name, opts=None):
    _initialize_spm()
...
def install(url, opts=None):
    opts = opts or {}
    _initialize_spm()

    pkg_name = _determine_package_name(url)
    _salt_print('Starting to install {0}...'.format(pkg_name))

    installed = _get_installed()

    if pkg_name in installed:
        if installed[pkg_name]['url'] == url:
            _salt_print('{0} is already installed!'.format(pkg_name))
        else:
            _salt_print('{0} is already installed from a different source.'.format(pkg_name))
    ...
    # Link all the files!
    _salt_print('Installing {0}...'.format(pkg_name))
    created_files = []
    ...
    _salt_print('{0} successfully installed!'.format(pkg_name))
